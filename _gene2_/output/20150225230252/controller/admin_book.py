...
def admin_book():
    ...
    if keyword:
        ...
        data['records'] = db.session.query(db_Book).\
            filter(and_(db_Book.flag==1, or_(db_Book.tel==keyword, db_Book.username==keyword, db_Book.name==keyword)))
    else:
        data['records']  = db_Book.query.filter_by(**cond).order_by('-id').limit(length).offset(offset)
    amount = db_Book.query.filter_by(**cond).count()
    
    data['pgnation'] = common.pagination(index, amount, length, common_flask.pagination_url, common_flask.pagination_jump)
    return render_template('admin/book_list.html', data=data)

@app.route('/admin/book/edit/<int:xid>', methods=['POST', 'GET'])
@requires_auth
def admin_book_edit(xid):
    if xid==0:
        record = {}
    else:
        record = db_Book.query.filter_by(flag=1, id=xid).first()
        if not record:
    ...
    if request.method == "GET":
        data = {}
        data['record'] = record
        return render_template('admin/book_edit.html', data=data)
...
@app.route('/admin/book/delete/<int:xid>', methods=['POST', 'GET'])
@requires_auth
def admin_book_delete(xid):
    if request.method == "GET":
        record = db_Book.query.filter_by(id=int(xid)).first()
        if not record:
            return default_error (u'该数据不存在')
        ...
        aa=db.session.query(db_Book).filter(db_Book.id.in_ (xids) ).update({'flag':0}, synchronize_session=False)
        db.session.commit ()
