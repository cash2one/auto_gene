...
from model.tables import policy as m_policy
...
def check_right (xid, user_id=get_user() ):
    return True
    #当前用户是否有权限
    return m_policy.exists (** { 'id': xid, 'create_user': user_id} )
...
class policy_read:
    ...
    def GET(self, xid):
        ...
        if not check_right (xid):
            print 'try to read an unauthrithm data, %s record id:%s , user id:%s'  %  ('policy',xid, get_user())
            raise web.notfound()
        record = m_policy.get_one (**{"id": int(xid)})
        return render ('admin/policy_read.html', data = {'record':record})
...
class policy_edit:
    ...
    def GET(self,xid):
        ...
        if xid and not check_right (xid):
            print 'try to edit unauthorization data, table:%s,  id:%s'  %   ( 'policy', xid)
            return default_error ()
        
        data = {}
        if xid:
            data['record'] = m_policy.get_one (**{"id": int(xid)})
            if not data:
                print 'Error, try to edit record but not found data, table:%s,  id:%s'   % ('policy', xid)
                raise web.notfound()
        return render ('admin/policy_edit.html', data = data)
    ...
    def POST(self,xid):
        ...
        request = web.input()
        input_fields = [   'policy_title',    'policy_keywords',    'policy_content',    'policy_categoryID',    'policy_areaID',    'policy_status',               ]
        nonul_fields = [   'policy_title',      'policy_content',    'policy_categoryID',    'policy_areaID',    'policy_status',               ]   #user input fileds, can not be emtpy
        
        #检查用户是否有权限
        if xid!=0 and not check_right (xid):
            print 'try to save an unauthrithm data, %s record id:%s , user id:%s'  %  ('policy',xid, get_user())
            raise web.notfound()
        
        #检查是否存在 不能为空的字段 输入为空
        if not self.check_input (request, nonul_fields):
            print 'try to edit data, but found some not-null parameter null, table: %s'  % 'policy'
            return default_error('some parameter empty')
        
        data = {}
        
        if xid==0:   #new record
            print 'add new record into database for table policy'
            data["id"] = 0
            data['create_time'] = get_date(); data['create_user'] = get_user()
        else:
            print 'update record into database for table policy'
            data = m_policy.get_one ( ** {'id': xid})
            if not data:
        ...
        m_policy.upsert ("id",**data)
        return web.seeother('/admin/policy'+"_list")
...
class policy_delete:
    ...
    def GET(self, xid):
        assert (str(xid).isdigit())
        if not check_right (xid):
            print 'try to delete an unauthrithm data, %s record id:%s , user id:%s'  %  ('policy',xid, get_user())
            raise web.notfound()
        m_policy.delete (**{"id":xid})
        return web.seeother('/admin/policy' + '_list')
