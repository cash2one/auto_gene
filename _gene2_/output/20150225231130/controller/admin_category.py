...
def admin_category():
    ...
    if keyword:
        ...
        data['records'] = db.session.query(db_Category).\
            filter(and_(db_Category.flag==1, or_(db_Category.tel==keyword, db_Category.username==keyword, db_Category.name==keyword)))
    else:
        data['records']  = db_Category.query.filter_by(**cond).order_by('-id').limit(length).offset(offset)
    amount = db_Category.query.filter_by(**cond).count()
...
def admin_category_edit(xid):
    if xid==0:
        ...
        record = db_Category.query.filter_by(flag=1, id=xid).first()
        if not record:
...
@app.route('/admin/category/delete/<int:xid>', methods=['POST', 'GET'])
@requires_auth
def admin_category_delete(xid):
    if request.method == "GET":
        record = db_Category.query.filter_by(id=int(xid)).first()
        if not record:
            return default_error (u'该数据不存在')
        ...
        aa=db.session.query(db_Category).filter(db_Category.id.in_ (xids) ).update({'flag':0}, synchronize_session=False)
        db.session.commit ()
