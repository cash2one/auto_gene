...
def admin_salary():
    ...
    if keyword:
        ...
        data['records'] = db.session.query(db_Salary).\
            filter(and_(db_Salary.flag==1, or_(db_Salary.tel==keyword, db_Salary.username==keyword, db_Salary.name==keyword)))
    else:
        data['records']  = db_Salary.query.filter_by(**cond).order_by('-id').limit(length).offset(offset)
    amount = db_Salary.query.filter_by(**cond).count()
    
    data['pgnation'] = common.pagination(index, amount, length, common_flask.pagination_url, common_flask.pagination_jump)
    return render_template('admin/salary_list.html', data=data)

@app.route('/admin/salary/edit/<int:xid>', methods=['POST', 'GET'])
@requires_auth
def admin_salary_edit(xid):
    if xid==0:
        record = {}
    else:
        record = db_Salary.query.filter_by(flag=1, id=xid).first()
        if not record:
    ...
    if request.method == "GET":
        ...
        temp = get_parameter(  'staff', 'level', 'money', 'flag', 'remark',  )
...
@requires_auth
def admin_salary_delete(xid):
    if request.method == "GET":
        record = db_Salary.query.filter_by(id=int(xid)).first()
        if not record:
            return default_error (u'该数据不存在')
